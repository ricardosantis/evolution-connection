...
export function useEvolutionApi({ addLog, setStep }: UseEvolutionApiProps) {
  ...
  const handleSubmit = async (formState: any) => {
    if (!formState.firstName || !formState.lastName || !formState.phone || formState.phone.length !== 8) {
    ...
    const payload = {
      instanceName,
      qrcode: true,
      number: `55${formState.ddd}9${formState.phone}`,
